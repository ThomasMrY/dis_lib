...
from methods.unsupervised import vae  # pylint: disable=unused-import
...
def train(model_dir,
  ...
  tpu_estimator = tf.estimator.Estimator(
  ...
  tpu_estimator.train(
      input_fn=lambda:input_fn(dataset, random_state.randint(2**32),batch_size),
...
def input_fn(ground_truth_data, seed, batch_size, num_batches=None):
